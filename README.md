...
```
**0.6.0:** This version has a dependency on `cats 0.8.1` so may not be compatible with projects depending on different versions of cats

**0.6.1:** This version has a dependency on `cats 1.0.1` so is only guaranteed to be compatible with projects depending on `cats 1.*.*` (see note above)
